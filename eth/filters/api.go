...
// NewPendingTransactionsDetail creates a subscription that is triggered each time a transaction
// enters the transaction pool and was signed from one of the transactions this nodes manages.
func (api *PublicFilterAPI) NewPendingTransactionsDetail(ctx context.Context) (*rpc.Subscription, error) {
	...
	go func() {
		...
		pendingTxSub := api.events.SubscribePendingTxsDetail(txDetails)

		for {
			select {
			case txDetails := <-txDetails:
				// To keep the original behaviour, send a single tx hash in one notification.
				// TODO(rjl493456442) Send a batch of tx hashes in one notification
				for _, h := range txDetails {
					notifier.Notify(rpcSub.ID, types.NewTransactionDetail(h))
				}
			case <-rpcSub.Err():
				pendingTxSub.Unsubscribe()
				return
